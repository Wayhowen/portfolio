...
export default function App() {
  ...
  return (
    ...
    <>
      {
        !locationIsHome &&
        <RxArrowLeft className="hidden sm:block fixed top-0 left-2 text-7xl z-10 text-white hover:scale-95 click:scale-95 cursor-pointer"
      ...
      <div className="bg-gradient-to-r from-black to-indigo-800 flex flex-row min-h-screen w-full">
        ...
        {location.pathname !== "/" && transitionStage === "animate-fadeIn" &&
          <nav
            className="basis-2/12 hidden md:flex flex-col justify-center items-center border-l border-indigo-500/50 rounded-full gap-y-4">
            <SmallHeaderButton mainText={"About me"} to="/me"
                               customCSS="outline-red-800 hover:bg-red-800/75 animate-homeSlideR4" index={0}
                               icon={<GrUserManager size={40} className="icon"/>}/>
            <SmallHeaderButton mainText={"Experience"} to="/experience"
                               customCSS="outline-green-800 hover:bg-green-800/75 animate-homeSlideR5" index={1}
                               icon={<GrDocumentText size={40} className="icon"/>}/>
            <SmallHeaderButton mainText={"Projects"} to="/projects"
                               customCSS="outline-blue-700 hover:bg-blue-700/75 animate-homeSlideR6" index={2}
                               icon={<GrIteration size={40} className="icon"/>}/>
            <SmallHeaderButton mainText={"Contact"} to="/contact"
                               customCSS="outline-yellow-500 hover:bg-yellow-500/75 animate-homeSlideR7" index={3}
                               icon={<GrContact size={40} className="icon"/>}/>
          </nav>
        }
      </div>
    </>
